	...
	public static final int NUM_TIME_SLOTS = 10;
	  ...
	  public void createRandomInstance(int nBuildings, int nRooms, int nCourses) {
	    ...
	    for (int i = 0; i < nBuildings; i++) {
	      ...
	      tmp.xCoor = random.nextDouble() * MAX_X_COORD;
	      tmp.yCoor = random.nextDouble() * MAX_Y_COORD;
	      buildings.add(tmp);
	    }
	  ...
	  public double evaluateSchedule(Schedule solutionSchedule) {
	    ...
	    for (int i = 0; i < s.length; i++) {
	      for (int j = 0; j < s[0].length; j++) {
	        ...
	        double xDist = (b1.xCoor - b2.xCoor) * (b1.xCoor - b2.xCoor);
	        double yDist = (b1.yCoor - b2.yCoor) * (b1.yCoor - b2.yCoor);
